...
uses
 ...
 //Android.JNI.PowerManager,
  ...
  IdExplicitTLSClientServerBase, IdFTP, FMX.TMSBaseControl,
  FMX.TMSTableView, FMX.DateTimeCtrls, FMX.TMSGridCell, FMX.TMSGridOptions,
  FMX.TMSGridData, FMX.TMSCustomGrid, FMX.TMSGrid, FMX.ExtCtrls,
  FMX.Gestures, FMX.Media, FMX.ScrollBox, FMX.ListView.Adapters.Base,
  System.Sensors.Components, FMX.TMSLed, System.ImageList, FMX.ImgList,
  Data.DBXJSONCommon,
  Data.FMTBcd,
  FMX.TMS7SegLed,
  FMX.TMSMatrixLabel,
...
type
  ...
  TfrmEmp_WorkStartCheck = class(TForm)
...
var
  frmEmp_WorkStartCheck: TfrmEmp_WorkStartCheck;
  p,q:String;
implementation
Uses
...
procedure TfrmEmp_WorkStartCheck.btnCloseClick(Sender: TObject);
begin
...
procedure TfrmEmp_WorkStartCheck.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TfrmEmp_WorkStartCheck.FormCreate(Sender: TObject);
var i:integer;
    LItem: TListViewItem;
begin
...
procedure TfrmEmp_WorkStartCheck.FormDestroy(Sender: TObject);
begin
{$IFDEF ANDROID}
...
procedure TfrmEmp_WorkStartCheck.FormKeyUp(Sender: TObject; var Key: Word; var KeyChar: Char; Shift: TShiftState);
begin
...
procedure TfrmEmp_WorkStartCheck.btnStartStopClick(Sender: TObject);
{$ifdef android}
...
procedure TfrmEmp_WorkStartCheck.p_Clear_BeaconList;
var i:Integer;
begin
...
procedure TfrmEmp_WorkStartCheck.p_Clear_AllValue(aStart:Boolean);
begin
  p_Clear_BeaconList;
  if l_Discovering then BluetoothLEManager.CancelDiscovery;
  FloatAnimation_Image.Stop;
...
procedure TfrmEmp_WorkStartCheck.p_Process;
begin
  txtProcess.Text := p+':'+q;
end;


procedure TfrmEmp_WorkStartCheck.onDiscoverLEDevice(const Sender: TObject; const ADevice: TBluetoothLEDevice;
...
function TfrmEmp_WorkStartCheck.f_Seconds(aDateTimeFr,aDateTimeTo:TDateTime):String;
begin
...
procedure TfrmEmp_WorkStartCheck.Timer1Timer(Sender: TObject);
var i,c,k:integer;
    v_Sec:integer;
...
procedure TfrmEmp_WorkStartCheck.p_LocationAddress(const aLocation: TLocationCoord2D);
var
...
procedure TfrmEmp_WorkStartCheck.OnGeocodeReverseEvent(const Address: TCivicAddress);
begin
{$IFDEF ANDROID}
...
procedure TfrmEmp_WorkStartCheck.p_checkSelfPermission(Sender: TObject);
begin
...
procedure TfrmEmp_WorkStartCheck.RequestPermissions;
begin
 {$IFDEF ANDROID}
...
procedure TfrmEmp_WorkStartCheck.HandlePermissionsRequest(const Sender: TObject; const M: TMessage);
begin
 {$IFDEF ANDROID}
...
{$IFDEF ANDROID}
procedure TfrmEmp_WorkStartCheck.OnPermissionsRequest(ARequestCode: Integer; APermissions: TJavaObjectArray<JString>; AGrantResults: TJavaArray<Integer>);
begin
...
initialization
   RegisterClasses([TfrmEmp_WorkStartCheck]);

finalization
   UnRegisterClasses([TfrmEmp_WorkStartCheck]);
